...
if (!isset($_SESSION['technician_id']) || !isset($_GET['user_id'])) {
    header("Location: inbox_technician.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($message !== null || $imagePath !== null) {
        $sql = "INSERT INTO messages (sender_id, receiver_id, message, image_path, created_at)
                VALUES (?, ?, ?, ?, NOW())";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iiss", $technician_id, $user_id, $message, $imagePath);
        $stmt->execute();
    }

    header("Location: chat_room_technician.php?user_id=$user_id");
    exit();
}
...
$stmt = $conn->prepare($sql);
$stmt->bind_param("iiii", $technician_id, $user_id, $user_id, $technician_id);
$stmt->execute();
...
<head>
  ...
  <title>ห้องแชทกับ <?= htmlspecialchars($user_name) ?></title>
...
<body>
  <div class="chat-container">
    <div class="chat-header">ห้องแชทกับ <?= htmlspecialchars($user_name) ?></div>
    <div class="chat-box" id="chatBox">
      <?php while ($msg = $messages->fetch_assoc()): ?>
        <div class="message <?= $msg['sender_id'] == $technician_id ? 'me' : 'other' ?>">
